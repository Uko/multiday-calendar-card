...
class MultiDayCalendarCard extends HTMLElement {
  ...
  private _dayHeaderHeight?: number;
  private _dayHeaderAnimationReady = false;
  ...
  private statusMessageMarkup(): string {
  ...
  private updateStatusMessage(): void {
    const existing = this.querySelector<HTMLElement>('.status-overlay');
    ...
    else this.querySelector<HTMLElement>('.schedule')?.insertAdjacentHTML('beforebegin', markup);

    this.querySelector<HTMLButtonElement>('.status-dismiss')?.addEventListener('click', () => {
      this._error = undefined;
      this.updateStatusMessage();
    }, { once: true });
  }
  ...
  private updateDayHeaderHeight(dayHeaderHeight: number): void {
    if (this._dayHeaderHeight === dayHeaderHeight) return;
    const card = this.querySelector<HTMLElement>('ha-card');
    card?.classList.toggle('header-height-animated', this._dayHeaderAnimationReady);
    const height = `${dayHeaderHeight}px`;
    this.querySelectorAll<HTMLElement>('.day-header, .time-axis-spacer, .time-axis-top-fade')
    ...
    if (!this._dayHeaderAnimationReady) {
      requestAnimationFrame(() => {
        if (this._dayHeaderHeight === dayHeaderHeight) {
          ...
          this.querySelector<HTMLElement>('ha-card')?.classList.add('header-height-animated');
        }
      });
    }
  }

  private updateHeaderForDays(days: readonly Date[]): void {
    this.updateDayHeaderHeight(calendarHeaderHeight(Math.max(
      0,
      ...days.map((day) => this.eventsForDay(day)
        .filter(({ event }) => allDayEventPlacementForDay(event, day) !== undefined)
        .length),
    )));
  }

  private updateHeaderForViewport(viewport?: HTMLElement): void {
  ...
  private updateLoadedDayColumns(dayKeys: readonly string[]): void {
    if (!this._config) return;
    ...
    for (const key of dayKeys) {
      const column = this.querySelector<HTMLElement>(`.day-column[data-day="${key}"]`);
      if (!column) continue;
      const [year, month, date] = key.split('-').map(Number);
      const day = new Date(year, month, date);
      const timeline = column.querySelector<HTMLElement>('.timeline');
      const header = column.querySelector<HTMLElement>('.day-header');
      if (!timeline || !header) continue;
      ...
      const allDayEvents = dayEvents
        ...
        .filter((item): item is { calendar: CalendarConfig; eventIndex: number; placement: NonNullable<typeof item.placement> } =>
        ...
        .join('');
      if (allDayEvents) header.insertAdjacentHTML('beforeend', `<div class="all-day-events">${allDayEvents}</div>`);
    ...
    if (!this._lookAroundScrollInProgress) {
      this.updateHeaderForViewport(this.querySelector<HTMLElement>('.calendar-viewport') ?? undefined);
    }
    this.updateNowLine();
  }
  ...
  private bindEventActions(root: ParentNode = this): void {
    if (this._config?.tap_action.action !== 'more-info') return;
    root.querySelectorAll<HTMLElement>('[data-event-index]').forEach((element) => {
  ...
  private bindLookAround(initialScrollPosition?: { left: number; top: number }): void {
    ...
    const calendarOrigin: CalendarScrollPoint = { x: 0, y: 0 };
    let startLeft = 0;
    let startTop = 0;
    let scrollGeometry: CalendarScrollGeometry | undefined;
    const measureScrollGeometry = (): CalendarScrollGeometry | undefined => {
      const horizontalAnchor = anchorColumn();
      const verticalAnchor = anchorVertical();
      if ((horizontal && !horizontalAnchor) || (vertical && !verticalAnchor)) return undefined;
      const viewportBounds = viewport.getBoundingClientRect();
    ...
    const isAtCalendarOrigin = (): boolean => {
      if (!scrollGeometry) return true;
      const point = rawScrollToCalendarPoint(
    ...
    const positionAtStartDay = (): boolean => {
      ...
      if (horizontal) viewport.scrollLeft = scrollPositionToRestore?.left ?? startLeft;
      if (vertical) viewport.scrollTop = scrollPositionToRestore?.top ?? startTop;
      ...
      requestAnimationFrame(() => {
        if (this._lookAroundInitialized) refreshGeometryAfterHeaderUpdate();
      });
      this._lookAroundResizeObserver?.disconnect();
      this._lookAroundResizeObserver = undefined;
      return true;
    ...
    viewport.addEventListener('scroll', () => {
      ...
      if (!initialized || this._lookAroundAnimating) return;
      if (isAtCalendarOrigin()) hideRecenterButton();
