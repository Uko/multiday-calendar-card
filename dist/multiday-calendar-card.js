...
class MultidayCalendarCardEditor extends HTMLElement {
    ...
    restoreScrollPositions() {
        ...
        if (!positions)
            return;
        const restore = () => positions.forEach(({ element, left, top }) => {
    ...
    renderLookAroundEditor() {
        const target = this.querySelector('[data-look-around-editor]');
        if (!target)
            return;
