...
function eventPlacementForDay(event, day, visibleStartMinutes, visibleEndMinutes) {
    ...
    const clippedEnd = new Date(Math.min(end.getTime(), visibleEnd.getTime()));
...
function rebaseRawScrollForGeometry(raw, previous, next) {
...
function normalizeLookAroundMode(value) {
    return typeof value === 'string' && LOOK_AROUND_MODES.includes(value)
        ? value
        : 'none';
}
/** Normalize the nested configuration into runtime defaults. */
function normalizeLookAroundSettings(value) {
...
function validateLookAroundSettings(value) {
    if (value === undefined)
        return [];
...
class MultidayCalendarCardEditor extends HTMLElement {
    ...
    bindEvents() {
        ...
        this.querySelector('[data-action="look-around-mode"]')?.addEventListener('change', (event) => {
            this.updateLookAround({ mode: event.target.value });
        });
        this.querySelector('[data-action="origin-snap"]')?.addEventListener('change', (event) => {
            this.updateLookAround({
                origin_snap_distance: event.target.checked ? LOOK_AROUND_ORIGIN_SNAP_DISTANCE_PX : 0,
            });
        });
        this.querySelector('[data-action="automatic-recenter"]')?.addEventListener('change', (event) => {
            this.updateLookAround({
                automatic_recenter: event.target.checked ? LOOK_AROUND_RESET_DELAY_MS / 1_000 : 0,
            });
        });
...
class MultiDayCalendarCard extends HTMLElement {
    ...
    setConfig(config) {
        ...
        if (lookAroundErrors.length > 0)
            throw new Error(lookAroundErrors.join(' '));
        const calendars = config.calendars ?? [];
    ...
    startDayRolloverTimer() {
        ...
        this._dayRolloverTimerId = window.setTimeout(() => {
            this._dayRolloverTimerId = undefined;
            if (this.isConnected && this.advanceStartDay(new Date()))
                void this.loadEvents(true);
            if (this.isConnected)
                this.startDayRolloverTimer();
    ...
    updateNowLine() {
        ...
        const { startMinutes, endMinutes } = lookAroundVerticalRange(this._config.look_around.mode, configuredStartMinutes, configuredEndMinutes);
    ...
    advanceStartDay(date) {
        if (!this._config)
            return false;
        if (!this.onNewStartDate(date))
            return false;
        this.render();
        return true;
    }
    ...
    normalDisplayDays() {
        if (!this._config)
            return [];
        return visibleDays(this._activeStartDay ?? this.resolveStartDay(), this._config.days, this._config.skip_days);
    }
    eventsForDay(day) {
        return this._eventsByDay.get(localDateKey(day)) ?? [];
    }
    async loadEvents(force = false, requestedDays) {
        ...
        if (force) {
            ...
            for (const key of this._eventsByDay.keys()) {
                if (!normalKeys.has(key) || targetKeys.has(key))
                    this._eventsByDay.delete(key);
            }
        }
        ...
        try {
            const eventGroups = await Promise.all(this._config.calendars.map(async (calendar) => ({
                calendar,
                events: await this._hass.callApi('get', buildCalendarEventsPath(calendar.entity, start, end)),
            })));
            if (generation !== this._eventCacheGeneration)
                return;
            const events = eventGroups.flatMap(({ calendar, events: calendarEvents }) => calendarEvents.map((event) => ({ calendar, event })));
    ...
    syncFixedVerticalLookAroundGeometry() {
        if (!this._config || this._config.height === null || !hasVerticalLookAround(this._config.look_around.mode))
            return;
        const configuredStart = parseTime(this._config.start_time);
        const configuredEnd = parseTime(this._config.end_time);
        if (configuredStart === undefined || configuredEnd === undefined)
            return;
        ...
        if (configuredMinutes <= 0)
            return;
        const viewport = this.querySelector('.calendar-viewport.native-vertical-time-axis');
        ...
        viewport.querySelectorAll(':scope > .time-axis, :scope > .day-columns')
            .forEach((element) => {
    ...
    updateLoadedDayColumns(dayKeys) {
        ...
        const { startMinutes, endMinutes } = lookAroundVerticalRange(config.look_around.mode, configuredStartMinutes, configuredEndMinutes);
    ...
    scheduleLookAroundReset(delaySeconds, reset) {
        if (delaySeconds === 0)
            return;
        if (this._lookAroundResetTimerId !== undefined)
            clearTimeout(this._lookAroundResetTimerId);
    ...
    bindLookAround(initialScrollPosition) {
        if (!this._config || this._config.look_around.mode === 'none')
            return;
        ...
        const measureScrollGeometry = () => {
            ...
            const verticalRange = lookAroundVerticalRange(this._config.look_around.mode, parseTime(this._config.start_time), parseTime(this._config.end_time));
        ...
        const releaseOriginLock = (deltaLeft, deltaTop) => {
            ...
            if (Math.abs(accumulatedOriginScrollLeft) < originSnapDistance &&
                Math.abs(accumulatedOriginScrollTop) < originSnapDistance)
                return;
            originLocked = false;
            if (horizontal)
                viewport.scrollLeft = startLeft + accumulatedOriginScrollLeft;
            if (vertical)
                viewport.scrollTop = startTop + accumulatedOriginScrollTop;
            accumulatedOriginScrollLeft = 0;
            accumulatedOriginScrollTop = 0;
        };
        const rebaseForGeometryChange = () => {
        ...
        const scheduleGeometryRefresh = () => {
            ...
            this._lookAroundGeometryAnimationFrameId = requestAnimationFrame(() => {
                this._lookAroundGeometryAnimationFrameId = undefined;
                if (this._lookAroundInitialized && !this._lookAroundAnimating)
                    rebaseForGeometryChange();
            });
        };
        this._lookAroundHeaderResizeObserver?.disconnect();
        this._lookAroundHeaderResizeObserver = new ResizeObserver(scheduleGeometryRefresh);
        const header = viewport.querySelector('.day-header');
        if (header)
            this._lookAroundHeaderResizeObserver.observe(header);
        ...
        viewport.addEventListener('touchmove', (event) => {
            ...
            if (touchX === undefined || touchY === undefined || lastTouchX === undefined || lastTouchY === undefined)
                return;
        ...
        const timeLabels = this.querySelector('.look-around-vertical-axis .time-labels');
        const timeAxisBottomFade = this.querySelector('.time-axis-bottom-fade');
        const setTimeAxisBottomFadeActive = (active) => {
            if (vertical)
    ...
    render() {
        ...
        const previousViewport = this._lookAroundInitialized
            ? this.querySelector('.calendar-viewport')
            : null;
        ...
        const timeLabelMinutes = [
            startMinutes,
            ...Array.from({ length: Math.max(0, Math.ceil((endMinutes - firstWholeHourAfterStart) / 60)) }, (_, index) => firstWholeHourAfterStart + index * 60).filter((minutes) => minutes < endMinutes),
            endMinutes,
        ];
