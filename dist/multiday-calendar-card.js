...
class MultiDayCalendarCard extends HTMLElement {
    ...
    animateLookAroundScroll(viewport, left, onComplete) {
        if (this._lookAroundAnimationFrameId !== undefined)
            cancelAnimationFrame(this._lookAroundAnimationFrameId);
    ...
    `;
        const recenterIcon = this.querySelector('.look-around-recenter ha-svg-icon');
        if (recenterIcon)
            recenterIcon.path = RECENTER_ICON_PATH;
